...
try {
  const { renderTemplate } = await import(join(root, "dist", "render-template.js"));
  ...
  if (renderedFiles.length !== 4) {
    throw new Error(`Expected 4 rendered files, received ${renderedFiles.length}.`);
  }

  const readme = await readFile(join(targetDirectory, "README.md"), "utf8");
  assertIncludes(readme, "# rendered-app", "rendered README");
  assertIncludes(readme, "TASK-005 replaces this placeholder", "rendered README");
  ...
  assertIncludes(marker, "BANKSTACK_PLACEHOLDER_BINARY_MARKER", "copied marker");

  const projectMarker = await readFile(join(targetDirectory, "rendered-app.txt"), "utf8");
  assertIncludes(projectMarker, "Rendered marker for rendered-app.", "rendered filename interpolation marker");

  const sourceMarker = await readFile(join(root, "templates", "placeholder", "assets", "marker.bin"));
  const copiedMarker = await readFile(join(targetDirectory, "assets", "marker.bin"));
